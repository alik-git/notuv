"""Tests for notuv command behavior."""
...
from notuv.config import NotuvConfig, NotuvError


def config(root: Path, *, editables: tuple[Path, ...] = ()) -> NotuvConfig:
    """Build a test config."""
    return NotuvConfig(
        project_root=root,
        entry_config_path=root / "notuv.toml",
        config_path=root / "notuv.toml",
        config_root=root,
        env_root=root,
        command_cwd=root,
...
def stack_config(root: Path, *, editables: tuple[Path, ...] = ()) -> NotuvConfig:
    ...
    return NotuvConfig(
        ...
        config_path=root / "notuv.mlab.toml",
        config_root=root,
        env_root=env_root,
        command_cwd=project_root,
        base_conda_env="mlab-shared",
        venv=env_root / ".notuv" / "mlab" / ".venv",
        editable_packages=editables,
        install_editable_deps=False,
        config_kind="stack",
    )
...
def test_find_git_root_fails_clearly(
    ...
    with pytest.raises(NotuvError, match="not inside a git worktree"):
...
def test_update_editables_installs_no_deps_by_default(
    ...
    calls: list[list[str]] = []
    monkeypatch.setenv("CONDA_EXE", "/opt/conda/bin/conda")
    monkeypatch.setattr("notuv.cli.make_status_path", lambda: status_path)
...
def test_run_conda_overlay_reports_conda_infrastructure_failure(
    ...
    with pytest.raises(NotuvError, match="conda failed before"):
...
def test_run_conda_overlay_requires_launcher_status(
    ...
    """Treat missing status plus successful conda as a notuv launcher bug."""
    status_path = tmp_path / "status.txt"
    monkeypatch.setenv("CONDA_EXE", "/opt/conda/bin/conda")
    monkeypatch.setattr("notuv.cli.make_status_path", lambda: status_path)
    ...
    with pytest.raises(NotuvError, match="did not report"):
        run_conda_overlay(config(tmp_path), ["python", "--version"])
...
def test_update_editables_raises_child_failure_without_conda_failure(
    ...
    with pytest.raises(NotuvError, match="exit code 9"):
        update_editables(config(tmp_path, editables=(package,)))
...
def test_launcher_records_missing_command_as_127(tmp_path: Path) -> None:
    ...
    completed = subprocess.run(  # noqa: S603
        [
            ...
            str(tmp_path),
            str(status_path),
            "definitely-not-a-notuv-test-command",
        ],
        text=True,
        capture_output=True,
        check=False,
...
def test_clean_refuses_shared_stack_venv_by_default(tmp_path: Path) -> None:
    ...
    with pytest.raises(NotuvError, match="refusing to remove shared stack venv"):
        clean(cfg)
...
def test_notuv_env_prefers_venv_bin(tmp_path: Path) -> None:
    """Put the worktree venv first on PATH."""
    env = notuv_env(config(tmp_path))
...
def test_print_info_includes_root_and_shared_venv_details(
    ...
    assert f"entry config: {tmp_path / 'minerva_lab' / 'notuv.toml'}" in out
    assert f"effective config: {tmp_path / 'notuv.mlab.toml'}" in out
...
def test_reject_editable_pip_install(args: list[str]) -> None:
    """Reject editable installs outside notuv.toml."""
    with pytest.raises(NotuvError, match="notuv update-editables"):
