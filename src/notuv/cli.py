"""Command-line interface for notuv."""
...
from notuv.config import NotuvConfig, NotuvError, load_config
...
try:
    ...
    print(f"notuv: command not found: {command[0]}", file=sys.stderr)
    write_status(127)
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run the notuv command-line interface."""
    args = list(sys.argv[1:] if argv is None else argv)
    try:
        return _main(args)
    except NotuvError as exc:
        print(f"notuv: {exc}", file=sys.stderr)
        return 2
    except KeyboardInterrupt:
        return 130
...
def find_git_root(cwd: Path) -> Path:
    ...
    if result.returncode != 0:
        raise NotuvError("not inside a git worktree")

    root = Path(result.stdout.strip())
    if not root:
        raise NotuvError("git did not return a worktree root")
    return root.resolve()


def ensure_venv(config: NotuvConfig) -> None:
...
def update_editables(config: NotuvConfig) -> None:
    ...
    for package in config.editable_packages:
        if not package.exists():
            raise NotuvError(f"editable package path does not exist: {package}")
...
def clean(config: NotuvConfig, *, allow_shared: bool = False) -> None:
    ...
    if config.uses_shared_venv and not allow_shared:
        raise NotuvError(
            ...
            f"  venv: {config.venv}\n\n"
            "Run `notuv clean --shared` if you intentionally want to remove it.",
        )
    if not config.venv.exists():
        print(f"Already clean: {display_path(config.venv, root=config.env_root)}")
        return
    if not config.venv.is_dir():
        raise NotuvError(f"venv path exists but is not a directory: {config.venv}")
    shutil.rmtree(config.venv)
    print(f"Removed {display_path(config.venv, root=config.env_root)}")


def print_info(config: NotuvConfig) -> None:
    """Print the active notuv configuration."""
...
def run_passthrough(config: NotuvConfig, args: list[str]) -> int:
    """Run a normal command inside conda with the worktree venv first on PATH."""
    return run_conda_overlay(config, args)


def run_conda_overlay(config: NotuvConfig, args: list[str]) -> int:
    ...
    try:
        try:
            ...
            completed = subprocess.run(  # noqa: S603
                ...
                env=notuv_env(config),
                check=False,
            )
        except FileNotFoundError as exc:
            raise NotuvError(f"command not found: {command[0]}") from exc

        if status_path.is_file():
            return read_exit_status(status_path)
        if completed.returncode != 0:
            raise NotuvError(
                "conda failed before the notuv command ran with exit code "
                f"{completed.returncode}:\n"
                f"  conda env: {config.base_conda_env}\n"
                f"  command: {_quote(args)}",
            )
        raise NotuvError(
            "notuv launcher did not report the command exit status",
        )
    finally:
...
def reject_editable_pip_install(args: list[str]) -> None:
    """Reject ad hoc editable installs managed outside notuv.toml."""
    ...
    raise NotuvError(
        "editable installs must be declared in notuv.toml and installed with "
        "`notuv update-editables`; do not run ad hoc `pip install -e ...` "
        "through notuv",
    )


def parse_clean_args(args: list[str]) -> bool:
    """Parse arguments for ``notuv clean``."""
    allow_shared = False
    for arg in args:
        if arg == "--shared":
            allow_shared = True
            continue
        raise NotuvError(f"unknown clean option: {arg}")
    return allow_shared
...
def run_checked(
    ...
    *,
    ...
    try:
        ...
        raise NotuvError(f"command not found: {args[0]}") from exc
...
def notuv_env(config: NotuvConfig) -> dict[str, str]:
    """Build the environment used for notuv-managed commands."""
    env = os.environ.copy()
...
def make_status_path() -> Path:
    ...
    handle, name = tempfile.mkstemp(prefix="notuv-status-", suffix=".txt")
    os.close(handle)
    path = Path(name)
    path.unlink()
    return path
...
def read_exit_status(status_path: Path) -> int:
    ...
    try:
        ...
        raise NotuvError(f"invalid notuv launcher exit status: {text!r}") from exc
    if returncode < 0:
        raise NotuvError(f"invalid notuv launcher exit status: {returncode}")
    return returncode
...
def conda_executable() -> str:
    ...
    if conda:
        return conda
    raise NotuvError("could not find conda; set CONDA_EXE or put conda on PATH")
